...
def find_even_index(arr)
  arr.each_with_index do |number, index|
    if index == 0
      left_sum = 0
      right_sum = arr[index + 1..-1].inject(:+)
    elsif index == arr.length - 1
      left_sum = arr[0..index - 1].inject(:+)
      right_sum = 0
    else
      left_sum = arr[0..index - 1].inject(:+)
      right_sum = arr[index + 1..-1].inject(:+)
    ...
    if left_sum == right_sum # covers primary use case
      return index
    end
  end

  return -1 # covers when there is no index where sums are equal
end
